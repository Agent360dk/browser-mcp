...
**Which half carries which fix.** This project is two pieces: a Chrome extension and an MCP server published on npm. The privacy fixes below — the action log, the cookie rules, tab adoption — live in the **extension**. Installing the new npm package does not change them: you get them when the extension itself is updated, which for a Chrome Web Store install happens after Google approves the new version (usually 1-3 days). The npm package refreshes the copy of the extension it ships, so an unpacked install can reload it straight away.
...
- `browser_get_cookies` without a `domain` returned every cookie in your Chrome profile, including httpOnly session cookies the page's own JavaScript cannot see. It now requires a domain, and returns only cookies for the http(s) pages the session has open.
