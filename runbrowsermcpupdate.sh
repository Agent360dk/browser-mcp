...
# Én vagt, to udgivelses-veje - se scripts/flow-daekning.sh for hvorfor.
. "$(dirname "$0")/scripts/flow-daekning.sh"
flow_nulstil_arv
...
if [[ "$SKIP_FLOW" == 1 || "${SPRING_FLOW_OVER:-}" == "1" ]]; then
  ...
  if ! grep -q "^DAEKNING:" "$FLOW_UD"; then
    ...
    elif ! grund="$(flow_daekning_ok "$FLOW_UD")"; then
      gate "flow-spaerren daekkede ikke alt: $grund Koer uden FLOW_KUN_BAGGRUND, eller acceptér hullet eksplicit."
    else
