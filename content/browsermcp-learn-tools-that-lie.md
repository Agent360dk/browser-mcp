...
⛔ **"Now" means on `main`, not in what you can install.** The fix is not in 1.30.0. We checked the published package rather than trusting the sentence above: `npm pack @agent360/browser-mcp@1.30.0`, unpacked, and the prototype setter is not in its `extension/background.js`. Install 1.30.0 today and the controlled select still does not land. Writing "we land it now" without this paragraph would have been the same class of overstatement the rest of this page is about - a sentence that is true of the code and false of the product - so it is worth saying plainly that we caught it in ourselves on the same day, by measuring instead of re-reading.
