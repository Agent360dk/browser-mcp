...
We measured it twice - on 19 September and again on 21 September, after we had fixed something the first run found in us. A controlled field, a controlled `<select>` and a file input, driven by each tool against the same fixture, with the page recording what it actually received. Nine measurements each time, and **no lies in any of the eighteen**: Playwright MCP landed all three, both times. Chrome DevTools MCP refused all three and said exactly why, both times. We landed the field and the file on 19 September and honestly failed the select; on 21 September we landed all three.
...
The uncomfortable line in that table used to be ours. On the controlled select - the exact case this whole release came from - Playwright landed the choice on 19 September and we did not. We answered honestly, which is better than the false yes we used to give, but honest and working are not the same thing.

**It is fixed, and re-measured on 21 September: we land it now.** One line did it. A controlled component puts a value setter on the *instance* that rolls a naive assignment back, so `sel.value = x` wrote the old value again and the component never heard anything. The prototype's setter goes around the instance and means the same thing. Five other places in the same file already did it that way - filling a field, clearing one, setting a date, typing into a combobox. `select_option` was the only one without the trick, and it was the only place an outside measurement found us.

⛔ **Retracted the same day. There was no React bug.** Everything in the two paragraphs above is about a failure we measured against our own test page, and our own test page was wrong. It put a value setter and a `_valueTracker` on a `<select>` and the comment above it called that "the same mechanism React uses". React does not do either of those things to a `<select>`: it installs a tracker on `input` and `textarea` only, and it reads a select's value on the native `change` event. Measured against real React 18.3.1 in jsdom, the plain assignment that 1.30.0 already makes lands the value and fires `onChange`.

So: **install 1.30.0 and a React controlled `<select>` works.** The one-line change is still in the code, because a component *could* do what our test page did, and then it matters - but we have not found a real library that does. We announced a hole in ourselves that was not there.

This is the same failure this page is about, pointing the other way. An instrument that is not calibrated against the real thing produces a confident number either way, and a false confession is not more honest than a false boast - it is just wrong in the direction that feels safer. The page you are reading had it live for a few hours. We are leaving the retraction here rather than editing the paragraphs above, because the sequence is the point.

**And the text field half is the same story.** Our page puts a value setter on the input that writes the *old* state back, and called that React's mechanism. React's own instance setter writes the new value and records it (`currentValue = '' + value` in `trackValueOnNode`), and it bails entirely if something already defined `value` on the element. Nothing in React rolls an assignment back. So read every number on this page as measured against **a hypothetical hostile component** - one that actively refuses what you write - and not against React. That is a fair thing to be robust against, and it is a fair measurement of exactly that. It is not a statement about React, and we made it one. A second consultant found this half after the first found the select half; neither saw the other's.

⛔ **Level with Playwright, not ahead of it.** Playwright landed all three before and after; the only thing that changed is that our own gap closed. Writing it any other way would be exactly the kind of overstatement this page is about.

⛔ **Retracted - the paragraph below is the claim this page took back.** It is kept because the sequence is the point, but do not read it as true: React does not put a value setter on a `<select>`, and the fixture that said otherwise was ours. Measured against real React 18.3.1, the plain assignment lands.

> **Update, 21 September - since retracted.** We found the cause, and it does us no credit. One line set the value the way a controlled component is built to refuse: `sel.value = opt.value`. React, and the fixture in this measurement, put a setter on the *instance* that rolls a naive assignment back, so the component never heard anything. The prototype's setter goes around the instance and means the same thing. **Five other places in the same file already did it that way** - filling a field, clearing one, setting a date, typing into a combobox. `select_option` was the only one without the trick. We had written the fix five times and missed the one place an outside measurement found us.

All three tools were re-run on the same fixture in the same session on 21 September, so every number above is current. The method and the raw output are in the repo at `test/aerlighed/RESULTAT-2026-09-21-tre-vejs.md`.
