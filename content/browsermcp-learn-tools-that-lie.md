...
It is fixed, and on 21 September we ran the measurement again against a real Chrome rather than only a unit test. Our own column now reads true-yes on all three cases - the controlled field, the controlled select, and the file input. The select lands.

⚠️ **We re-ran only our own column.** Playwright MCP and Chrome DevTools MCP were not measured again, so every number in the table above is still the one from 19 September, and it stays that way until all three are run on the same fixture in the same session. "We pass our own fixture" is a different claim from "we beat them", and a page about tools that overstate themselves is the worst possible place to blur the two. The method and the raw output are in the repo at `test/aerlighed/RESULTAT-2026-09-21.md`.
