...
**Fix status: shipped in v1.29.1.** The Cmd+A clear shipped in **v1.24.0** and the typing rewrite in **v1.25.0**, and we said both halves were fixed. They were not: on 2026-09-08 the append came back on a React form, because the keystroke clear does not empty a controlled field. The fix (read the field back, clear it through the native setter if it is still full) shipped in v1.29.1. (An earlier release note claimed the typing rewrite had already shipped when it had not; we corrected the record rather than let it stand, and then actually shipped it.)
