...
| A `<select>` inside a controlled component (React and friends) | **Open in 1.30.0** | The value never lands. A controlled component puts a value setter on the *instance* that rolls a naive assignment back, and `select_option` was the one place in the file that assigned naively - five other paths already went through the prototype's setter. Found 2026-09-19 by an outside measurement, not by us. One line fixes it, and that line is on `main`; it is not in 1.30.0, which is what you can install today. This row says **Fixed** when a release carries it, and not before. |
