...
import { SceneLabel, AnimatedCursor, ClickRipple, GlowHighlight } from "./components/Effects";
...
const SplitLayout: React.FC<{
  ...
  // Content visible from frame 0, fades in over 15 frames (under the scene label)
  const fadeIn = spring({ frame: Math.max(0, frame - 5), fps, config: { damping: 12 } });
      ...
      {label && (
        <div
          style={{
            ...
            alignItems: "center",
            gap: 10,
            opacity: interpolate(frame, [28, 38], [0, 1], { extrapolateRight: "clamp" }),
          }}
        >
        ...
        browser={
          <Browser url="dashboard.stripe.com/apikeys">
            ...
            {showDashboard && (
              <>
                <StripeDashboard />
                <GlowHighlight x={555} y={98} width={180} height={22} visible={frame >= 100} color="#635bff" />
                <AnimatedCursor positions={[
                  { x: 400, y: 300, frame: 55 },
                  { x: 650, y: 110, frame: 95 },
                ]} />
              </>
            )}
      ...
      <SplitLayout
        ...
        browser={
          <Browser url="console.cloud.google.com/auth/branding">
            ...
            />
            <AnimatedCursor positions={[
              { x: 400, y: 200, frame: 10 },
              { x: 350, y: 135, frame: 40 },
              { x: 350, y: 175, frame: 65 },
              { x: 350, y: 218, frame: 98 },
              { x: 250, y: 280, frame: 130 },
            ]} />
            <ClickRipple x={350} y={135} triggerFrame={32} />
            <ClickRipple x={250} y={280} triggerFrame={148} />
          </Browser>
        }
      />
    </>
        ...
        terminal={
          <Terminal
            lines={[
              ...
              { text: "", type: "output", delay: 95 },
              { text: '✓ User entered: "847291"', type: "success", delay: 95 },
              { text: 'fill("#otp", "847291")', type: "command", delay: 102 },
              { text: 'press_key("Enter")', type: "command", delay: 112 },
              { text: "✓ Logged in! Continuing...", type: "success", delay: 120 },
            ]}
          />
        }
        browser={
          <Browser url="app.hubspot.com/login/2fa">
            <div style={{ width: "100%", height: "100%", background: "#f5f8fa", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui" }}>
              <div style={{ background: "#fff", borderRadius: 8, padding: 32, boxShadow: "0 2px 8px rgba(0,0,0,0.1)", textAlign: "center", width: 300 }}>
                ...
                <GlowHighlight x={80} y={130} width={140} height={36} visible={showCode && !showSuccess} color="#22c55e" />
              </div>
            </div>
            <AskUserOverlay
...
const Scene4LinkedIn: React.FC = () => {
  const frame = useCurrentFrame();
  const messageText = "Hej Lars — jeg så dit opslag om AI i salg. Vi har bygget en voice agent der booker 20+ møder/md for teams som jeres. Har du 15 min?";
      ...
      <SplitLayout
        ...
        terminal={
          <Terminal
            lines={[
              ...
              { text: 'fill("[contenteditable]", "Hej Lars...")', type: "command", delay: 48 },
        ...
        browser={
          <Browser url="linkedin.com/in/lars-hansen">
            ...
            <GlowHighlight x={180} y={138} width={330} height={32} visible={frame >= 42 && !sent} color="#0a66c2" />
            <AnimatedCursor positions={[
              { x: 350, y: 100, frame: 30 },
              { x: 350, y: 155, frame: 42 },
            ]} />
            {sent && (
              <div style={{
        ...
        terminal={
          <Terminal
            lines={[
              ...
              { text: "✓ Deploy complete!", type: "success", delay: 115 },
            ]}
          />
        }
        browser={
          <Browser url="railway.com/project/agent360dk">
            ...
            <GlowHighlight x={150} y={115} width={200} height={25} visible={frame >= 110} color="#22c55e" />
          </Browser>
        }
      />
    </>
...
const EndCard: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: "#0a0a0a", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
      <div style={{ fontSize: 42, fontWeight: 700, color: "#fff", fontFamily: "system-ui", opacity: fadeIn }}>
...
export const Demo: React.FC = () => {
  const S = 160; // tighter scenes (~5.3 sec)
  const T = 90;  // title
  const E = 120; // end card

  let f = 0;
  const seq = (dur: number) => { const from = f; f += dur; return from; };

  const s0 = seq(T);
  const s1 = seq(S + 20); // navigate (slightly longer)
  const s2 = seq(S + 20); // click+fill
  const s3 = seq(S);       // 2FA
  const s4 = seq(S);       // LinkedIn
  const s5 = seq(S - 20);  // network (shorter)
  const s6 = seq(S);       // multi-session
  const s7 = seq(E);       // end

  return (
    <AbsoluteFill>
      <Sequence from={s0} durationInFrames={T}><TitleCard /></Sequence>
      <Sequence from={s1} durationInFrames={S + 20}><Scene1Navigate /></Sequence>
      <Sequence from={s2} durationInFrames={S + 20}><Scene2ClickFill /></Sequence>
      <Sequence from={s3} durationInFrames={S}><Scene3HumanInLoop /></Sequence>
      <Sequence from={s4} durationInFrames={S}><Scene4LinkedIn /></Sequence>
      <Sequence from={s5} durationInFrames={S - 20}><Scene5Network /></Sequence>
      <Sequence from={s6} durationInFrames={S}><Scene6MultiSession /></Sequence>
