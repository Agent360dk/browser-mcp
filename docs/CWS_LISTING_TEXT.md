...
|---|---|---|
| Summary (the only text visible without expanding) | "Control your real Chrome from Claude Code — navigate, click, fill, screenshot, solve CAPTCHAs. 43 tools, multi-session." | Never mentions that a local MCP server is required. A store-only installer lands on "Not connected" with no idea why. |
